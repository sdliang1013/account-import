...
public class ExcelDataReader {
	 ...
	 * @return 组装后的Excel数据
	 */
	public static ExcelData readExcel(ExcelStruct excelStruct, InputStream importExcelStream, int sheetIndex,
                                      Map<String, Object> params, boolean needValidation) throws IOException {
		...
		if(repeatImportCells == null || repeatImportCells.size() <= 0 || StringUtils.isEmpty(excelStruct.getEndCode())) {
		...
		// 7.
        if (needValidation) {
		    ValidateUtil.processValidate(excelData, params);
        }
		return excelData;
	}
	...
	private static int readRepeatImportData(List<ImportCellDesc> repeatImportCells, Sheet sheet, FormulaEvaluator evaluator, String endCode, ExcelData excelData) {
		...
		if(repeatImportCells == null || repeatImportCells.size() <= 0 || StringUtils.isEmpty(endCode) || sheet == null || sheet.getLastRowNum() <= 0) {
			return repeatCount;
		}
	...
	private static void readOnceImportData(List<ImportCellDesc> onceImportCells, Sheet sheet, FormulaEvaluator evaluator, ExcelData excelData) {
		...
		for(ImportCellDesc desc : onceImportCells) {
			if(desc == null || StringUtils.isEmpty(desc.getCellRef())) {
				continue;
			}
			// 获取单元格
